  ...
  const checkHealth = async () => {
    ...
    try {
      const checkHealth = async () => {
        setIsLoading(true);
        try {
          // UPDATED: Use the dynamic URL here too
          ...
          // If the fetch completely fails, the Express server is completely down
          setStatus({
            backend: 'down',
            database: 'down',
            llm: 'down',
      ...
      useEffect(() => {
        checkHealth();
        // Optional: Auto-refresh the status every 30 seconds
        const interval = setInterval(checkHealth, 30000);
        return () => clearInterval(interval);
      }, []);

      // Helper component for the status indicator circles
